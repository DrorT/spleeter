...
class AudioProcessor {
  ...
  async loadAudio(file) {
    try {
      ...
      // Decode audio data
      let audioBuffer = await this.decodeAudioData(arrayBuffer);

      // Resample if source sample rate differs from target (e.g., 48000 -> 44100)
      if (this.sampleRate && audioBuffer.sampleRate !== this.sampleRate) {
        this.logger.warn("AudioProcessor", "Resampling audio", {
          from: audioBuffer.sampleRate,
          to: this.sampleRate,
        });
        audioBuffer = await this.resampleAudioBuffer(
          audioBuffer,
          this.sampleRate
        );
      }
  ...
  /**
   * Resample an AudioBuffer to target sample rate using OfflineAudioContext
   * @param {AudioBuffer} sourceBuffer
   * @param {number} targetRate
   * @returns {Promise<AudioBuffer>}
   */
  async resampleAudioBuffer(sourceBuffer, targetRate) {
    // If already desired rate, return
    ...
    try {
      ...
      this.logger.debug("AudioProcessor", "Resampling completed", {
        originalRate: sourceBuffer.sampleRate,
        targetRate,
      ...
      this.logger.error(
        "AudioProcessor",
        "Resampling failed, using original buffer",
        e
      );
      return sourceBuffer; // fallback
    }
  }
